...
// verifyCommonHeaders validates that common middleware headers are present in the response.
// This includes correlation ID (from CorrelationID middleware) and CORS headers (from CORS middleware).
//
// Validated headers:
...
//   - t: Testing instance
//   - response: The HTTP response to validate
func verifyCommonHeaders(t *testing.T, response *httptest.ResponseRecorder) {
	t.Helper()
