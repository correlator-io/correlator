...
// ==============================================================================
// Double-Prefix Safety: Anchored Name-Transformation Patterns
// ==============================================================================

// TestResolver_AnchoredPattern_NoDoublePrefix verifies that anchored patterns
// generated by the pattern suggester for same-namespace name transformations
// do NOT corrupt canonical URNs by applying the transformation twice.
//
// This is the critical safety property: the source pattern must only match
// orphan URNs (short form), not canonical URNs (long form with prefix).
func TestResolver_AnchoredPattern_NoDoublePrefix(t *testing.T) {
	if !testing.Short() {
		t.Skip("skipping unit test in non-short mode")
	}

	// Pattern generated by SuggestPatterns for:
	...
	testCases := []struct {
		...
		{
			name:     "orphan transformed to canonical",
			input:    "postgresql://demo-postgres/marts.customers",
			expected: "postgresql://demo-postgres/demo.marts.customers",
		},
		{
			name:     "canonical unchanged (no double-prefix)",
			input:    "postgresql://demo-postgres/demo.marts.customers",
			expected: "postgresql://demo-postgres/demo.marts.customers",
		},
		{
			name:     "different anchor unchanged",
			input:    "postgresql://demo-postgres/staging.orders",
			expected: "postgresql://demo-postgres/staging.orders",
		},
		{
			name:     "different namespace unchanged",
			input:    "postgresql://other-host/marts.customers",
			expected: "postgresql://other-host/marts.customers",
		},
	}
